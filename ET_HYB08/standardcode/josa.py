...
def get_table(info:list, path:str, news_code:str,code:str)->str:
	# 표를 위한 데이터를 구한다
	# 폰트 사이즈 및 배경 색 조정
	font_size_px = 23
	font_size_pt = 13
	...
	get_color(table_data1, info[0][2], info[0][4], 'white')
	get_color(table_data2, info[1][2], info[1][4], '#474747')
	get_color(table_data3, info[2][2], info[2][4], '#474747')
	get_color(table_data4, info[3][2], info[3][4], '#474747')
	get_color(table_data5, info[4][2], info[4][4], '#474747')
	...
	TODAY = datetime.today().strftime("%Y%m%d")
	print(path)
	img='{}\{}{}_{}.jpeg'.format(path, TODAY, news_code, code)	
	plt.savefig(img)
	plt.close('all')
	return img


def get_color(table, fore, org, b):
	font_size_pt = 13
	data_font_size = font_size_pt
	edge_colors = '#474747'
	# edge_colors = '#C9C9C9'
...
def get_rise_drop(ratio):
	...
	if float(ratio) == 0.0:
		...
		r = f"-{str(abs(ratio))}% 하락"
		return r
	else:
		r = f"+{str(abs(ratio))}% 상승"
		return r
	

def rise_fall_mark(ratio):
	if float(ratio) == 0.0:
		return str(ratio)+'%'
	elif float(ratio) < 0:
		r ='-'+str(ratio)[1:] + '%'
...
def signal_make_para(name:str,residue:str):	
	#name, josa, rate, month, date, finish
	PARA1="{}{} {}월 {}일 매도신호가 포착돼 수익률 {}%를 기록"
	#name, josa, month, date, price,finish
	PARA2="AI분석한 결과에 따르면 {}{}  {}월 {}일 매수가 {}으로 매수신호가 포착"
	eun=def_connect_word(name)[0][-1]
	if len(residue)>=16:		
		price=numberToKoreanWon(residue[:-9].replace(' ',''))
		date=date=residue[-9:]
		month=date[5:-2]
		day=date[7:]		
		para=PARA2.format(name, eun, month, day, price)		
		return para
	elif len(residue)<=15:				
		rate=residue[:-9]
		rate=rate.replace(' ','')
		...
		month=date[5:-2]
		day=date[7:]		
		para=PARA1.format(name, eun, month, day, rate)		
		return para

def write_log(path, name, something):
    f = open("{}{}.txt".format(path,name), 'w', encoding='utf-8')
