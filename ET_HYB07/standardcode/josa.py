...
def get_table(info:list, path:str, news_code:str,code:str):
	# 표를 위한 데이터를 구한다
	# 폰트 사이즈 및 배경 색 조정
	font_size_px = 23
	font_size_pt = 13
	...
	get_color(table_data2, info[1][2], info[1][4], '#ffffff')
	get_color(table_data3, info[2][2], info[2][4], '#ffffff')
	get_color(table_data4, info[3][2], info[3][4], '#ffffff')
	get_color(table_data5, info[4][2], info[4][4], '#ffffff')
	...
	# 차트 이미지를 저장한다
	TODAY = datetime.today().strftime("%Y%m%d")
	img = f'{path}\{TODAY}{news_code}_{code}.jpeg'
	plt.savefig(img)
	plt.close('all')
	return img


def get_color(table, fore, org, color):
	font_size_pt = 13
	data_font_size = font_size_pt
	edge_colors = '#C9C9C9'
	font_colors = color
	...
	for j, cell in six.iteritems(table._cells):
		...
		if j[1] == 0:
			cell.set_text_props(fontproperties=fontprop, color='#474747')
			cell._loc = 'center'
		...
		if j[1] == 1:			
			if fore < 0:				
				...
				cell.set_text_props(weight='normal', color='#474747')
		if j[1] == 2:
			if org > 0:
				cell.set_text_props(weight='normal', color=red_color)
			elif org < 0:
...
def get_separated_date(dt, dot=False):
    if not dt or dt == '-':
        return '-'
    y = dt[:4]
    m = dt[5:6]
